...
void _read_proc(xlator_t *this)
{
        ...
        for (;;)
        {
                ...
                /* Convert this req to a feature vector and 
                        * put it into temporal 'req2vec' hashtable for subsequent quering.
                        * generate a pointer to the cached metadata if it exists.
                        *  
                        * log buffer.
                */
...
init (xlator_t *this)
{
        ...
        conf->buffer = rb_create(DEFAULT_TABLE_SIZE, DEFAULT_BUFFER_SIZE, "/root/test/log.json");
        if(!conf->buffer)
                goto out;
        /*----Thread init----*/
        /* 
